...
typedef struct
{
        ...
        uint64_t write_back_addr, prefetch_addr;
        uint64_t c, b, s, k;
        uint64_t lines, ways;
} Cache;
...
uint64_t Cache_execute_prefetch(Cache *pCache, cache_stats_t* p_stats)
{
  ...
  if(d == pCache->pending_stride)
  {
    ...
    {
       uint64_t addr = pCache-> prefetch_addr + (i * pCache->pending_stride);
       pCache->clock++;
       if(Cache_prefetch(pCache, addr) == MISS)
        count++;
       if(pCache->write_back){
        pCache->write_back = false;
        p_stats->write_backs++;
       }
    }
  }
...
void cache_access(char rw, uint64_t address, cache_stats_t* p_stats) {
  ...
  //Now we need L2
  if(status == MISS){
    CacheStatus l2_status;
    if(l1.write_back){
      l2_status = Cache_write(&l2, l1.write_back_addr);
      l1.write_back = false;
